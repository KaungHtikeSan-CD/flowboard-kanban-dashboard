...
export default function Dashboard({ tasks, categories }) {
  ...
  const categoryCounts = categories.map((category) => ({ label: category, value: tasks.filter((task) => task.category === category).length }))
  ...
  const completedTasks = Object.values(performance).reduce((sum, value) => sum + value, 0)
  const completed = completedTasks || 1
  return <><div className="page-intro"><div><p className="eyebrow">Insights</p><h1>Project dashboard</h1><p className="subtext">A quick view of your team’s progress.</p></div></div>
    ...
    <div className="charts-grid"><section className="chart-card"><div className="chart-title"><div><h2>Task status</h2><p>Work distribution across the board</p></div></div>{tasks.length ? <div className="donut-area"><div className="donut" style={{ background: `conic-gradient(${segments})` }}><span><strong>{tasks.length}</strong><small>tasks</small></span></div><div className="legend">{statuses.map((item) => <div key={item.key}><i style={{ background: item.color }} />{item.label}<strong>{item.value}</strong></div>)}</div></div> : <p className="empty-state">Create your first task to see the status chart.</p>}</section>
      ...
      <section className="chart-card performance-card"><div className="chart-title"><div><h2>Completion performance</h2><p>Finished tasks compared with their due dates</p></div></div>{completedTasks ? <div className="performance-bars">{Object.entries(performance).map(([label, value]) => <div key={label}><div className="performance-track"><i className={label.toLowerCase().replace(' ', '-')} style={{ height: `${value / completed * 100}%` }} /></div><strong>{value}</strong><span>{label}</span></div>)}</div> : <p className="empty-state">Move a task to Done to see completion performance.</p>}</section></div>
  </>
}
